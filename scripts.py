...
def train_scene(gpu, loss, factor):
    ...
    for fold in [0,1,2,3,4]:
        ...
        cmd = f"nnUNetv2_ratio_estimator --biomarker=ntr --ce_type=bins15 {gt_folder} {pred_folder}"
        print(cmd)
        if not dry_run:
            os.system(cmd)
        # test
        cmd = f"nnUNetv2_ratio_estimator --biomarker=ntr --ce_type=bins15 {gt_folder.replace('labelsVal_ece', 'labelsTs')} {pred_folder.replace('validation_ece','test')}"
        print(cmd)
        if not dry_run:
            os.system(cmd)

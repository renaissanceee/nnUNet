...
def analyze_r_ce_std(tensor_nec_prob_map, tensor_wt_prob_map, tensor_nec_gt_map, tensor_wt_gt_map, tensor_seg_prob_map,
    ...
    if 'kde' in ce_type:
        ...
        ce_left, ce_right = get_ce_bound(y_bar, x_bar, mean_l2_y, mean_l2_x)
    elif ce_type == 'nll':
...
def calc_v_bias(tensor_nec_prob_map, tensor_wt_prob_map, tensor_nec_gt_map, tensor_wt_gt_map):
    v_bias_y = mean_lp_dist(tensor_nec_prob_map.reshape(-1), tensor_nec_gt_map.reshape(-1).to(torch.int64), p=1)
    v_bias_x = mean_lp_dist(tensor_wt_prob_map.reshape(-1), tensor_wt_gt_map.reshape(-1).to(torch.int64), p=1)
...
def compute_estimator(reference_file: str, prediction_file: str, probability_file: str,
    ...
    # V-Bias: y,x
    results['ratio']['v_bias']['v_bias_y'] = v_bias_y
    results['ratio']['v_bias']['v_bias_x'] = v_bias_x
    # CE: y,x
...
def compute_estimator_avg(reference_file: str, prediction_file: str, probability_file: str,
    ...
    # results['ratio']['r_naive']['r_bias'] = r_naive-r_gt
    # # [x,y]
    ...
    # results['ratio']['r_1_ord_corr']['r_bias'] = r_1_ord_corr - r_gt
    ...
    # results['ratio']['r_2_ord_corr']['r_bias'] = r_2_ord_corr - r_gt
...
def compute_estimator_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    # bias for y and x
    mean_v_bias = {}
    paired_samples['v_bias']['v_bias_y'] = np.array([item['ratio']['v_bias']['v_bias_y'] for item in results])
    paired_samples['v_bias']['v_bias_x'] = np.array([item['ratio']['v_bias']['v_bias_x'] for item in results])
    mean_v_bias['v_bias_y'] = np.mean(paired_samples['v_bias']['v_bias_y'])
    mean_v_bias['v_bias_x'] = np.mean(paired_samples['v_bias']['v_bias_x'])
    # failure

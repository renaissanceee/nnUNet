...
losses = ['CELoss'] # ['CELoss', 'DiceLoss', '','Topk10','DiceTopK10']
...
def train_scene(gpu, loss, factor):
    ...
    for fold in [0,1,2,3,4]:
        ...
        # #---------------- after_TS #----------------
        gt_folder = f'{gt_root}/labelsTs/fold_{fold}/'
        pred_folder = f'{pred_root}/nnUNetTrainer{loss}__nnUNetPlans__2d/fold_{fold}/test'
        # test
        cmd = f"nnUNetv2_ratio_estimator --biomarker=ntr --ece_percentage={tile} --ce_type={ce_type} {gt_folder} {pred_folder}"
        print(cmd)
        if not dry_run:
